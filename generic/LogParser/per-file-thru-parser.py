...
def calcThroughput(i, complete_set, complete_dict):
    ...
    """
    thru = 0
    size = complete_dict[i][0]
    time = complete_dict[i][1]
    if time:
        thru = float(size / time) * 8
    else:
        thru = -1
    return thru
...
def main(logfile, csvfile):
    ...
    tmp_str = 'prodindex, throughput (bps)' + '\n'
    w.write(tmp_str)
    for i in rx_success_set:
        thru = calcThroughput(i, rx_success_set, rx_success_dict)
        tmp_str = str(i) + ',' + str(thru) + '\n'
        w.write(tmp_str)
    w.close()
